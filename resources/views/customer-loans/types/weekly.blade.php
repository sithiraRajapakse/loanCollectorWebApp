...
@section('title') Weekly Loan @endsection

@section('content')

    @component('common-components.breadcrumb')
        @slot('title') Register New Weekly Loan @endslot
        @slot('li_1') @endslot
    @endcomponent
        ...
        <div class="col-lg-5">
            <div class="card border border-success">
                <div class="card-body">
                    ...
                    <form action="{{ route('customer-loans.weekly.create') }}" method="post">
                        @csrf
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-6">
                                <div class="form-group">
                                    <label for="installments_count" required>No. of weeks</label>
                                    <div class="controls">
            ...
            <div class="card border border-primary">
                ...
                <div class="card-body p-0">
                    <table class="table table-striped">
                        ...
                        <tr>
                            <td>Name</td>
                            <td customer-placeholder-name>Not Available</td>
                        </tr>
                        <tr>
                            <td>Telephone No.</td>
                            <td customer-placeholder-telephone>Not Available</td>
                        </tr>
                        <tr>
                            <td>Address</td>
                            <td customer-placeholder-address>Not Available</td>
                        </tr>
                        <tr>
                            <td>Location</td>
                            <td customer-placeholder-location>Not Available</td>
                        </tr>
                        </tbody>
                    </table>
                </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>You are about to create a new <strong>Weekly Loan</strong>. Please check the following details and
...
@section('script-bottom')
    ...
    <script>
        ...
        $('#installments_count').change(function () {
            ...
            let ed = moment(sd).add(installmentCount, 'weeks');
            $('#end_date').val(ed.format('YYYY-MM-DD'));
        });

        function calculateWeeks(callback) {
            let startDate = moment($('#start_date').val());
            let endDate = moment($('#end_date').val());
            let days = endDate.diff(startDate, 'weeks');
        ...
        function calculate() {
            ...
            let noOfWeeks = $('#installments_count').val();
            console.log(`Number of weeks ${noOfWeeks}`);
            let tempEndDate = moment(_st);
            tempEndDate.add(noOfWeeks, 'weeks');
            ...
            noOfMonths = Math.ceil(noOfWeeks / 4); // no of months
            ...
            let installmentList = getInstallmentWeeksWithoutHolidays(startDate, noOfWeeks);
        ...
        function getInstallmentWeeksWithoutHolidays($startDate, $installmentsCount) {
            // get the installments tentative start and end dates
            let start_date = moment($startDate);
            let end_date = moment($startDate)

            end_date.add($installmentsCount - 1, 'weeks');

            let weeksPeriod = moment.range(start_date, end_date);
            let _installments = [];
            let _holidays = [];
            let _day = 0;

            for (let week of weeksPeriod.by('week')) {
                let td = week;
                // installment date is a holiday
                if (holidays.includes(week.format('YYYY-MM-DD'))) {
                    while(holidays.includes(td.format('YYYY-MM-DD'))) {
                        _holidays.push(td.format('YYYY-MM-DD'));
                        _installments.push({
